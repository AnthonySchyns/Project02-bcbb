                    ...
                    <?php  if (count($errors) > 0) : ?>
                        <?php foreach ($errors as $error) : ?>
                            <span class="error text-center font-weight-bold text-danger ml-1" style="font-size:10px">
                                <?php
                                if($error === "Email requis")echo $error;
                                if($error === "L'email existe déjà")echo $error;
                                ?>
                            </span>
                        <?php endforeach ?>
                     <?php  endif ?>
                    ...
                    <label for="nickname">
                        ...
                        <?php  if (count($errors) > 0) : ?>
                            <?php foreach ($errors as $error) : ?>
                                <span class="error text-center font-weight-bold text-danger ml-1" style="font-size:10px">
                                    <?php
                                        if($error === "Pseudo requis")echo $error;
                                        if($error === "Ce pseudo existe déjà")echo $error;
                                    ?>
                                </span>
                            <?php endforeach ?>
                        <?php  endif ?>
                    ...
                    <label for="password">
                        ...
                        <?php  if (count($errors) > 0) : ?>
                            <?php foreach ($errors as $error) : ?>
                                <span class="error text-center font-weight-bold text-danger ml-1" style="font-size:10px">
                                    <?php
                                        if($error === "Mot de passe requis")echo $error;
                                        if($error === "Les deux mots de passe ne correspondent pas")echo $error;
                                    ?>
                                </span>
                            <?php endforeach ?>
                        <?php  endif ?>
